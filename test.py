...
agent_params_file = sys.argv[1] + "/hyperparameters.yml"

with open(agent_params_file, 'r') as f:
    hyperparams = yaml.safe_load(f)

agent_file = hyperparams['FILE_NAME'] +'.pt'
...
actor = Actor(obs_dim, action_dim, hyperparams['ACTOR_HIDDEN_LAYERS'], action_range, None, hyperparams['STD_CLAMP_MIN'], hyperparams['STD_CLAMP_MAX'])
...
with open(sys.argv[1] + "/TrainingData.csv", 'w', newline = '') as f:
    writer = csv.writer(f)
...
with open(sys.argv[1] + "/TrainingDataSummary.txt", 'w', newline = '') as f:
    f.write(summary)


env.close()
env = gym.make(hyperparams['ENV_NAME'], render_mode='rgb_array')
env = RecordVideo(env, video_folder=sys.argv[1], episode_trigger=lambda x: True)
